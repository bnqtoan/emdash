...
The runtime's `collectPageAccess` is exposed on `Astro.locals.emdash` (on both the anonymous fast path and the full request path) so `<EmDashGate>` can resolve the verdict during render — without this wiring the gate always allowed, since the component reads `locals.emdash.collectPageAccess`.
